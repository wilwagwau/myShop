				...
				<section class="content">
					<div class="row">
						...
						<?php
							...
							if($row['numRows'] < 1)
							{
								echo '<h1 class="page-header">No results found for &#96;<i style="color:red">'.$_POST['keyword'].'</i>&#96;</h1>';
							}
							else
							{
								echo '<h1 class="page-header">Search results for &#96;<i style="color:lightseagreen">'.$_POST['keyword'].'</i>&#96;</h1>';
								try
								{
								$inc = 3;	
									$stmt = $conn->prepare("SELECT * FROM products WHERE name LIKE :keyword ORDER BY price ASC");
									$stmt->execute(['keyword' => '%'.$_POST['keyword'].'%']);
